 ...
 *      Output: 5.00000
 *      Explanation:
 *        1) The first customer arrives at time 1, the chef takes his order and starts preparing it immediately at time 1, and finishes
 *            at time 3, so the waiting time of the first customer is 3 - 1 = 2.
 ...
 *      Output: 3.25000
 *      Explanation:
 *        1) The first customer arrives at time 5, the chef takes his order and starts preparing it immediately at time 5, and finishes
 *            at time 7, so the waiting time of the first customer is 7 - 5 = 2.
...
export const averageWaitingTime = function (customers) {
  let finishedTime = 0;
  let waitingTime = 0;
  const totalCustomers = customers.length;
  for (let i = 0; i < totalCustomers; i++) {
    let times = customers[i];
    let arrive = times[0];
    let prepareTime = times[1];

    if (finishedTime > arrive) {
      finishedTime = finishedTime + prepareTime;
      waitingTime += finishedTime - arrive;
    } else {
      finishedTime = arrive + prepareTime;
      waitingTime += finishedTime - arrive;
    }
  }
